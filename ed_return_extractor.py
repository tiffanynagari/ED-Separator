...
# ---- GitHub auto-push (no manual download/upload) ---------------------------
# Fine-grained PAT with "Contents: Read and write" on this one repo only.
# Paste it here each session (or set it as a Colab "Secret" - see notes below).
GITHUB_TOKEN = ""                              # <-- paste your token here
GITHUB_REPO = "tiffanynagari/ED-Separator"      # owner/repo
GITHUB_FILE_PATH = "dashboard-data.json"        # path in the repo
GITHUB_BRANCH = "main"
...
# ---- 4b. Push dashboard-data.json straight to GitHub -------------------------
def push_to_github(json_str):
    import requests, base64

    if not GITHUB_TOKEN:
        print("\nGITHUB_TOKEN is empty — skipping auto-push. Paste your token "
              "into the config section to enable this.")
        return False
    ...
    # Need the current file's sha to update it (GitHub requires this for overwrites)
    get_res = requests.get(api_url, headers=headers, params={"ref": GITHUB_BRANCH})
    ...
    if put_res.status_code in (200, 201):
        print(f"\nPushed dashboard-data.json to {GITHUB_REPO} ({GITHUB_BRANCH}). "
              f"GitHub Pages will update in ~1 minute.")
        return True
    else:
        print(f"\nGitHub push failed ({put_res.status_code}): {put_res.text[:300]}")
        return False
...
def main():
    ...
    pushed = push_to_github(json_str)
    if not pushed:
        # Fallback: offer a manual download if GITHUB_TOKEN wasn't set
        try:
            files.download("dashboard-data.json")
        except Exception:
            pass
